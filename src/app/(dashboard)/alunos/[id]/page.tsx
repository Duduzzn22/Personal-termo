...
export default async function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const studentInvitations = (await invitations.list(userId)).filter(
    (inv: { student_id: string }) => inv.student_id === id
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <StudentDetailClient student={student} invitations={studentInvitations} />
    </div>
  );
}
